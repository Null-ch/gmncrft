...
домен (Caddy) — страницы открыты всем, скачивание файлов просит простой пароль (см. 6.1).
...
├── caddy-entrypoint.sh                      # хеширует DOWNLOAD_PASSWORD при старте caddy
...
`link-server` (см. `docker-compose.yml`) отдаёт HTML-страницы и файлы. Наружу он сам не
публикуется: перед ним стоит `caddy`, который добавляет HTTPS (домен) и спрашивает
пароль **только при реальном скачивании файла** — сами страницы открыты всем.
...
При переходе на сам файл (`.../backup/file` или `.../client/file`, их дёргают и
JS на страницах, и бот) браузер спросит логин `mc` и пароль — значение
`DOWNLOAD_PASSWORD` из `.env`. Это простой, не секретный пароль: Discord-бот
печатает его прямо в ответе на `/minecraft backup`/`/minecraft client`.
...
4. Запустить/перезапустить: `docker compose up -d`. Caddy сам получит сертификат
   ...
   и сам посчитает bcrypt-хеш `DOWNLOAD_PASSWORD` при старте (`caddy-entrypoint.sh`) —
   руками ничего хешировать не нужно. Проверить: `docker compose logs -f caddy`.

Смена пароля: поменяйте `DOWNLOAD_PASSWORD` в `.env` → `docker compose up -d caddy` →
не забудьте обновить `MINECRAFT_DOWNLOAD_PASSWORD` в `.env` бота и передеплоить его.

> `BACKUP_LINK_TOKEN` в `.env` — отдельный технический секрет между `caddy` и
> `link-server` внутри docker-сети (наружу не виден), не связан с `DOWNLOAD_PASSWORD`
> и трогать его без необходимости не нужно.
