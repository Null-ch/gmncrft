...
├── resourcepacks/                      # текстур-пак и русификатор модов .zip (только в клиент-пак, папка resourcepacks/)
...
| Деревни и мобы | Minecraft Comes Alive, Millager, Civillis, Cube Animals, UntitledDuck, Wolf Saddle-Bag, Gamingbarn's Morphs, Alex's Mobs Continued 2.2.2, Flying Unicorns |
...
## Пакеты ресурсов (текстуры и русификатор)

Пакеты ресурсов (`.zip`) лежат в `resourcepacks/` и так же, как шейдеры, кладутся в архив
для клиента в папку `resourcepacks/` без распаковки, а инструкция добавляется в `README.txt`
и на `/client`. На сервер не копируются, модов не требуют. После изменения `resourcepacks/` —
`docker compose restart link-server`. Сейчас их два:

- [Faithful 32x](https://modrinth.com/resourcepack/faithful-32x) `Faithful 32x - 26.1.zip` —
  ванильный стиль в 32x. Меняет только ванильные блоки и мобов, поэтому выбран пак в
  ванильном стиле — так модовые блоки не выбиваются.
- `Russian_Mods_Translation.zip` — русификатор модов: файлы `ru_ru.json` для 64 модов, у
  которых русского перевода не было или он неполный (Village Quest, R3CT Daily Quests,
  реплики жителей MCA, Cluttered, JEI, карты Xaero и др.). Если у мода свой перевод есть,
  пак только дописывает недостающие строки. Текст, который мод пишет прямо в коде, а не в
  языковых файлах, остаётся английским. После обновления или добавления модов их новые
  строки будут на английском, пока пак не пересобран.

### Инструкция для игроков: как подключить пакеты ресурсов

1. Возьми все `.zip` из папки `resourcepacks` архива и **не распаковывай их**.
2. Положи эти `.zip` как есть в папку `AppData\Roaming\.minecraft\resourcepacks`
...
4. В левом списке («Доступные») наведи на пакет и нажми стрелку — он переместится в
   правый список («Выбранные»). Так же перенеси остальные и нажми **Готово**.
5. Для русификатора в игре должен быть выбран язык «Русский» (**Настройки → Язык**).
